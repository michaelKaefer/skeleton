...
printf "2/5 Starting docker services...\n"
if ! OUTPUT="$( docker-compose up -d 2>&1 )"
then
  echo "2/5 Error on starting docker services:"
  echo "$OUTPUT"
  exit
fi
printf "2/5 Success\n\n"
...
printf "3/5 Starting built in Symfony server...\n"
...
then
  ...
  if echo "$OUTPUT" | grep "The local web server is already running for this project at port 8000" >/dev/null; then
    printf "3/5 Success, server is already listening on https://127.0.0.1:8000/\n\n"
  else
    echo "3/5 Error on starting Symfony server:"
    echo "$OUTPUT"
    exit
  fi
fi
...
printf "4/5 Starting browser-sync for hot reloading...\n"
...
if [ "$(uname)" == "Darwin" ]; then
  ...
  if BROWSER_SYNC_PROCESSES=$(ps -eo "[PID] %p  [COMMAND] %a" --sort=start_time | grep "[b]rowser-sync start"); then
      printf "4/5 Error on starting browser-sync: browser-sync is already running, please kill all existing browser-sync processes:\n\n"
...
then
  printf "4/5 Error on starting browser-sync: \n\n%s" "$OUTPUT"
  exit
fi
printf "4/5 Success, started brwoser-sync on https://127.0.0.1:3000/, for details use \"cat ./var/.browser-sync\"\n\n"


printf "5/5 Starting assets build watcher...\n"
...
if [ "$(uname)" == "Darwin" ]; then
  ...
  if YARN_ENCORE_PROCESSES=$(ps -eo "[PID] %p  [COMMAND] %a" --sort=start_time | grep "[y]arn.js encore dev"); then
      printf "4/5 Error on starting assets build watcher: yarn encore is already running, please kill all existing yarn encore processes:\n\n"
...
then
  echo "5/5 Error on starting assets build watcher:"
  echo "$OUTPUT"
  exit
fi
printf "5/5 Success\n\n"
...
printf "\t\t[USER]   \"root\"\n"
printf "\t\t[PW]     \"\"\n"
printf "\t\t[DB]     \"mygts_globaltcad_com\"\n"
printf "\tphpMyAdmin:\n"
