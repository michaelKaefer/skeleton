...
printf "1/7 Updating the symfony executable...\n"
if OUTPUT="$( sudo symfony self:update )"
then
  printf "1/7 Success\n\n"
fi

printf "2/7 Changing working directory to app directory...\n"
APP_PATH="$( cd "$( dirname "${BASH_SOURCE[0]}")" >/dev/null 2>&1 && pwd )"
if ! cd "$APP_PATH/.."
then
  echo "2/7 Error on changing working directory to app directory..."
  echo "$OUTPUT"
  exit
fi
printf "2/7 Success, changed working directory to %s\n\n" "$APP_PATH/.."

printf "3/7 Starting docker services...\n"
if ! OUTPUT="$( docker-compose up -d 2>&1 )"
then
  echo "3/7 Error on starting docker services:"
  echo "$OUTPUT"
  exit
fi
printf "3/7 Success\n\n"

printf "4/7 Starting built in Symfony server...\n"
...
then
  ...
  if echo "$OUTPUT" | grep "The local web server is already running for this project at port 8000" >/dev/null; then
    printf "4/7 Success, server is already listening on https://127.0.0.1:8000/\n\n"
  else
    echo "4/7 Error on starting Symfony server:"
    echo "$OUTPUT"
    exit
  fi
fi

printf "5/7 Starting browser-sync for hot reloading...\n"
if [ "$(uname)" == "Darwin" ]; then
  ...
  if BROWSER_SYNC_PROCESSES=$(ps -eo "[PID] %p  [COMMAND] %a" --sort=start_time | grep "[b]rowser-sync start"); then
      printf "5/7 Error on starting browser-sync: browser-sync is already running, please kill all existing browser-sync processes:\n\n"
...
then
  printf "5/7 Error on starting browser-sync: \n\n%s" "$OUTPUT"
  exit
fi
printf "5/7 Success, started brwoser-sync on https://127.0.0.1:3000/, for details use \"cat ./var/.browser-sync\"\n\n"

printf "6/7 Starting assets build watcher...\n"
if [ "$(uname)" == "Darwin" ]; then
    printf "Darwin not supported"
    exit
elif [ "$(expr substr $(uname -s) 1 5)" == "Linux" ]; then
  if YARN_ENCORE_PROCESSES=$(ps -eo "[PID] %p  [COMMAND] %a" --sort=start_time | grep "[y]arn.js encore dev"); then
      printf "5/7 Error on starting assets build watcher: yarn encore is already running, please kill all existing yarn encore processes:\n\n"
...
then
  echo "6/7 Error on starting assets build watcher:"
  echo "$OUTPUT"
  exit
fi
printf "6/7 Success\n\n"

printf "7/7 Starting React application in spa/...\n"
if ! cd "spa/"
then
  echo "7/7 Error on changing working directory to spa/..."
  echo "$OUTPUT"
  exit
fi
...
then
  ...
  echo "7/7 Error on starting React application in spa/:"
  echo "$OUTPUT"
  exit
fi
